...
import (
	...
	"github.com/Diniboy1123/usque/internal/congestion/common"
)
...
func (b *bbrSender) SetMaxDatagramSize(s congestion.ByteCount) {
	if s < b.maxDatagramSize {
		// Allow MTU to decrease (can happen during path MTU discovery)
		// Adjust congestion window proportionally
		if b.congestionWindow > 0 && b.maxDatagramSize > 0 {
			b.congestionWindow = congestion.ByteCount(float64(b.congestionWindow) * float64(s) / float64(b.maxDatagramSize))
